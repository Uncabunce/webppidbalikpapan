...
<main class="bg-surface min-h-screen">
    ...
    <div class="text-center mb-10">
        <span class="inline-block text-[10px] font-extrabold tracking-widest uppercase text-secondary bg-secondary-container/20 border border-secondary-container px-4 py-1 rounded-full mb-3">
            Pemerintah Kota Balikpapan
        </span>
        <h2 class="font-headline font-extrabold text-2xl md:text-3xl text-primary mb-2">
            Struktur Organisasi PPID
        </h2>
        <p class="text-sm text-outline max-w-xl mx-auto">
            Pejabat Pengelola Informasi dan Dokumentasi Utama Kota Balikpapan
        </p>
    </div>

    {{-- Org Chart Tree --}}
    <div class="flex flex-col items-center gap-0 font-body select-none">

        {{-- ─── PEMBINA ─── --}}
        @include('mega menu.profil-ppid.org-node', [
            'role'    => 'Pembina',
            'name'    => 'Wali Kota Balikpapan',
            'color'   => 'gold',
        ])
        ...
        {{-- ─── LEVEL 2 : Pengarah + Tim Pertimbangan ─── --}}
        <div class="relative flex items-start justify-center gap-16 w-full max-w-xl">

            {{-- Horizontal bar --}}
            <div class="absolute top-0 left-1/4 right-1/4 h-0.5 bg-outline-variant"></div>
            ...
            <div class="flex flex-col items-center">
                ...
                @include('mega menu.profil-ppid.org-node', [
                    'role'  => 'Pengarah',
                    'name'  => 'Sekretaris Daerah Kota Balikpapan',
                    'color' => 'navy',
                ])
            </div>
            ...
            <div class="flex flex-col items-center">
                ...
                @include('mega menu.profil-ppid.org-node', [
                    ...
                    'name'  => 'Kepala Perangkat Daerah Se-Kota Balikpapan',
                    'color' => 'teal',
                ])
            </div>
        </div>

        {{-- Converge connectors → PPID Utama --}}
        <div class="flex items-end w-full max-w-xl">
            <div class="flex-1 h-0.5 bg-outline-variant"></div>
            <div class="w-0.5 h-0.5 bg-outline-variant"></div>
            <div class="flex-1 h-0.5 bg-outline-variant"></div>
        </div>
        <div class="w-0.5 h-7 bg-outline-variant"></div>
        ...
        @include('mega menu.profil-ppid.org-node', [
            'role'  => 'PPID Utama',
            'name'  => 'Kepala Dinas Komunikasi dan Informatika Kota Balikpapan',
            'color' => 'navy',
            'wide'  => true,
        ])
        ...
        @include('mega menu.profil-ppid.org-node', [
            'role'  => 'PPID Pelaksana',
            'name'  => 'Pejabat Pengelola Informasi dan Dokumentasi Pelaksana Se-Kota Balikpapan',
            'color' => 'purple',
            'wide'  => true,
        ])
        ...
        {{-- ─── LEVEL 4 : 4 Bidang ─── --}}
        <div class="relative flex items-start justify-center gap-4 w-full max-w-4xl flex-wrap md:flex-nowrap">

            {{-- Horizontal bar (hidden on mobile) --}}
            <div class="hidden md:block absolute top-0 left-[12%] right-[12%] h-0.5 bg-outline-variant"></div>

            @foreach([
                ['role' => 'Sekretaris',             'name' => 'Sekretaris Diskominfo Kota Balikpapan',                                       'color' => 'navy'],
                ['role' => 'Pengelolaan Informasi',  'name' => 'Kepala Bidang Pengendalian Pembiayaan dan Pelaporan Bappeda Litbang Kota Balikpapan', 'color' => 'navy'],
                ['role' => 'Pelayanan Info & Dok.',  'name' => 'Kepala Bidang Informasi dan Komunikasi dan Diskominfo',                        'color' => 'teal'],
                ['role' => 'Pengaduan & Sengketa',   'name' => 'Kepala Bagian Hukum Setda Kota Balikpapan',                                   'color' => 'purple'],
            ] as $node)
                <div class="flex flex-col items-center">
                    <div class="w-0.5 h-6 bg-outline-variant"></div>
                    @include('mega menu.profil-ppid.org-node', $node)
                </div>
            @endforeach
        </div>

        {{-- Converge → Pejabat Fungsional --}}
        <div class="flex items-end w-full max-w-4xl">
            <div class="flex-1 h-0.5 bg-outline-variant"></div>
            <div class="w-0.5 h-0.5 bg-outline-variant"></div>
            <div class="flex-1 h-0.5 bg-outline-variant"></div>
            <div class="flex-1 h-0.5 bg-outline-variant"></div>
        ...
        {{-- ─── PEJABAT FUNGSIONAL ─── --}}
        <div class="rounded-xl border-2 border-secondary-container bg-secondary-container/10 px-6 py-4 text-center min-w-[220px]">
            <span class="inline-block text-[10px] font-extrabold tracking-widest uppercase bg-secondary-container text-on-secondary-container px-3 py-1 rounded-full mb-3">
                Pejabat Fungsional
            </span>
            <ul class="space-y-1 text-left mt-1">
                @foreach(['Pranata Humas', 'Pranata Komputer', 'Arsiparis'] as $i => $jabatan)
                    <li class="flex items-center gap-2 text-xs text-on-surface">
                        <span class="w-5 h-5 rounded-full bg-secondary-container text-on-secondary-container text-[10px] font-bold flex items-center justify-center shrink-0">
                            {{ $i + 1 }}
                        </span>
                        {{ $jabatan }}
                    </li>
    ...
    {{-- Legend --}}
    <div class="mt-12 flex flex-wrap justify-center gap-4 text-xs text-outline">
        @foreach([
            ['color' => 'bg-secondary-container text-on-secondary-container',    'label' => 'Pembina / Fungsional'],
            ['color' => 'bg-primary text-on-primary',                            'label' => 'PPID Utama / Pengarah / Bidang'],
            ['color' => 'bg-tertiary-container text-on-tertiary-container',       'label' => 'Tim Pertimbangan / Pelayanan'],
            ['color' => 'bg-[#534AB7] text-white',                               'label' => 'PPID Pelaksana / Pengaduan'],
        ] as $legend)
