...
<main class="bg-surface min-h-screen">
    <section class="relative py-20 overflow-hidden border-b border-outline-variant/10">
        ...
        <div class="max-w-4xl mx-auto px-6 relative z-10 text-center">
            <nav class="flex justify-center mb-4 text-xs font-medium text-white/60 uppercase tracking-widest gap-2">
                ...
                <span>Profil</span>
                <span>/</span>
                <span class="text-white">Makhlumat Pelayanan</span>
            </nav>
    ...
    <div class="relative rounded-2xl overflow-hidden mb-10 shadow-md">
        <div class="absolute inset-0 bg-primary opacity-95"></div>
        <div class="absolute inset-0" style="background-image:url('/batik%20dayak.png');background-repeat:repeat;background-size:200px;opacity:0.05;"></div>
        <div class="relative z-10 px-8 py-8 text-center">
            <span class="material-symbols-outlined text-secondary-container text-4xl mb-3 block">verified</span>
            <h2 class="font-headline font-extrabold text-white text-xl md:text-2xl mb-3">
                Maklumat Pelayanan Informasi Publik
            </h2>
            <p class="text-white/70 text-sm max-w-2xl mx-auto leading-relaxed">
                Selaku penyelenggara layanan informasi dan pelayanan penanganan pengaduan publik
                Pemerintah Kota Balikpapan, kami berkomitmen untuk:
            </p>
        </div>
    </div>
    ...
    <div class="space-y-4">
        @foreach([
            [
                'icon'  => 'speed',
                'title' => 'Pelayanan Cepat, Tepat, dan Akurat',
                'desc'  => 'Memberikan pelayanan permohonan informasi publik dan pelayanan penanganan pengaduan atas pelayanan publik yang diselenggarakan oleh Pemerintah Kota Balikpapan dengan cepat, tepat, akurat, dan tidak menyesatkan.',
                'color' => 'primary',
            ],
            [
                'icon'  => 'monetization_on',
                'title' => 'Prosedur Mudah dan Bebas Biaya',
                'desc'  => 'Memberikan pelayanan informasi dan penanganan pengaduan sesuai prosedur yang mudah dan bebas biaya.',
                'color' => 'tertiary',
            ],
            [
                'icon'  => 'lock',
                'title' => 'Kerahasiaan Data Pribadi',
                'desc'  => 'Menjaga kerahasiaan data pribadi pemohon informasi dan pelapor pengaduan publik.',
                'color' => 'primary',
            ],
            [
                'icon'  => 'groups',
                'title' => 'Petugas Kompeten dan Profesional',
                'desc'  => 'Menempatkan petugas pelayanan informasi dan pelayanan penanganan pengaduan yang kompeten, profesional, responsif, dan mengedepankan etika pelayanan.',
                'color' => 'tertiary',
            ],
            [
                'icon'  => 'assessment',
                'title' => 'Evaluasi Berkala dan Akuntabel',
                'desc'  => 'Melakukan evaluasi kinerja secara berkala atas penyelenggaraan pelayanan informasi dan pelayanan penanganan pengaduan serta siap menerima sanksi apabila melanggar peraturan perundang-undangan yang berlaku.',
                'color' => 'primary',
            ],
        ] as $i => $item)
        <div class="flex items-start gap-4 rounded-xl border
            {{ $item['color'] === 'tertiary' ? 'border-tertiary-container bg-surface-container-lowest' : 'border-primary-container bg-surface-container-lowest' }}
            px-5 py-4 shadow-sm hover:shadow-md transition-shadow">
            {{-- Number + icon --}}
            <div class="flex flex-col items-center gap-1 shrink-0">
                <div class="w-9 h-9 rounded-full flex items-center justify-center
                    {{ $item['color'] === 'tertiary' ? 'bg-tertiary-container' : 'bg-primary' }}">
                    <span class="material-symbols-outlined text-base
                        {{ $item['color'] === 'tertiary' ? 'text-on-tertiary-container' : 'text-on-primary' }}">
                        {{ $item['icon'] }}
                    </span>
                </div>
                <span class="text-[9px] font-extrabold
                    {{ $item['color'] === 'tertiary' ? 'text-tertiary' : 'text-primary' }}">
                    0{{ $i + 1 }}
                </span>
            </div>
            {{-- Text --}}
            <div class="flex-1 min-w-0">
                <h3 class="font-headline font-extrabold text-sm
                    {{ $item['color'] === 'tertiary' ? 'text-tertiary' : 'text-primary' }}
                    mb-1">
                    {{ $item['title'] }}
                </h3>
                <p class="text-xs text-on-surface leading-relaxed">{{ $item['desc'] }}</p>
            </div>
        </div>
        @endforeach
    </div>
    ...
    {{-- Signature seal --}}
    <div class="mt-10 rounded-xl border border-secondary-container bg-secondary-container/10 px-6 py-5 flex items-center gap-4">
        <span class="material-symbols-outlined text-secondary text-3xl shrink-0">workspace_premium</span>
        <p class="text-xs text-on-surface leading-relaxed">
            Maklumat ini merupakan komitmen resmi <strong class="font-semibold">PPID Utama Pemerintah Kota Balikpapan</strong>
            dalam memberikan pelayanan informasi publik yang transparan, akuntabel, dan berorientasi pada kepuasan masyarakat,
            sesuai dengan ketentuan Undang-Undang Nomor 14 Tahun 2008 tentang Keterbukaan Informasi Publik.
        </p>
    </div>
