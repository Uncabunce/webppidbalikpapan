...
<div class="max-w-7xl mx-auto px-4 md:px-6 flex flex-col md:flex-row gap-8 mt-8">
    ...
    <aside class="w-full md:w-1/4 flex-shrink-0">
        <div class="bg-white border border-outline-variant/50 rounded-lg p-5 shadow-sm sticky top-32">
            ...
            <div class="mb-4">
                ...
                <select id="filter-kategori" class="w-full border border-outline-variant/50 rounded text-sm text-on-surface focus:ring-primary focus:border-primary p-2.5 bg-surface-bright outline-none">
                    <option value="">Semua</option>
                    <option value="Berita Foto">Berita Foto</option>
                    <option value="Kegiatan Wali Kota">Kegiatan Wali Kota</option>
                    <option value="Infrastruktur">Infrastruktur</option>
                </select>
            </div>
            <div class="mb-6">
                <label class="block text-sm font-semibold text-on-surface mb-2 font-headline">Tanggal Berita</label>
                <input type="date" id="filter-tanggal" class="w-full border border-outline-variant/50 rounded text-sm text-on-surface focus:ring-primary focus:border-primary p-2.5 bg-surface-bright outline-none">
            </div>
            <button onclick="resetFilter()" class="w-full bg-surface-container hover:bg-surface-container-high text-on-surface font-semibold py-2.5 rounded transition-colors text-sm border border-outline-variant/30">
                Reset
            </button>
        </div>
    </aside>
    ...
    <section class="w-full md:w-3/4">
        <!-- Grid Foto -->
        <div id="foto-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
...
<footer class="bg-slate-800 w-full pt-16 md:pt-20 pb-8 border-t border-slate-600">
<div class="grid grid-cols-1 md:grid-cols-4 gap-12 max-w-7xl mx-auto px-4 md:px-6 text-sm leading-relaxed">
<div class="md:col-span-2">
<div class="text-xl md:text-2xl font-bold text-yellow-600 mb-6 font-headline">PPID Kota Balikpapan</div>
<p class="text-slate-400 max-w-md mb-8">Pejabat Pengelola Informasi dan Dokumentasi (PPID) Utama Pemerintah Kota Balikpapan bertanggung jawab untuk penyimpanan, pendokumentasian, penyediaan, dan pelayanan informasi publik.</p>
<div class="w-full rounded-xl overflow-hidden shadow-md" style="aspect-ratio:16/9;">
<iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d63821.253001534766!2d116.75573272167965!3d-1.2764074999999773!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x2df1473bb9404897%3A0x94d96108f98b652c!2sBalikpapan%20Mayor&#39;s%20Office!5e0!3m2!1sen!2ssg!4v1776063788871!5m2!1sen!2ssg" width="100%" height="100%" style="border:0;" allowfullscreen="" loading="lazy"></iframe>
</div>
</div>
<div>
<h4 class="font-headline font-bold text-yellow-600 text-lg mb-7">Kontak Kami</h4>
<ul class="space-y-4 text-slate-400">
<li class="flex gap-3"><span class="material-symbols-outlined text-secondary shrink-0">location_on</span><span class="text-xs md:text-sm">Jl. Jenderal Sudirman No.1, Klandasan Ulu, Balikpapan Kota, Kota Balikpapan, Kalimantan Timur 76112</span></li>
...
<ul class="space-y-3">
<li><a class="text-slate-400 hover:text-blue-300 underline-offset-4 hover:underline text-xs md:text-sm" href="#">Whistleblower System</a></li>
<li><a class="text-slate-400 hover:text-blue-300 underline-offset-4 hover:underline text-xs md:text-sm" href="#">Lapor!</a></li>
<li><a class="text-slate-400 hover:text-blue-300 underline-offset-4 hover:underline text-xs md:text-sm" href="#">Data Terbuka</a></li>
<li><a class="text-slate-400 hover:text-blue-300 underline-offset-4 hover:underline text-xs md:text-sm font-semibold" href="#">Kebijakan Privasi</a></li>
</ul>
</div>
</div>
<div class="max-w-7xl mx-auto px-4 md:px-6 mt-16 pt-8 border-t border-slate-600 text-center">
<p class="text-slate-400 text-[10px] md:text-xs">© 2024 Pemerintah Kota Balikpapan. PPID Utama. All rights reserved.</p>
</div>
</footer>
...
// =============================================
// DATA FOTO
// Ganti array ini dengan data dari API jika ada
// =============================================
// Generate 72 item otomatis (= 8 halaman)
var judulList = [
    'Sosialisasi Keterbukaan Informasi Publik',
    'Kunjungan Kerja Wali Kota',
...
var katList = ['Berita Foto', 'Kegiatan Wali Kota', 'Infrastruktur'];
var bulanList = ['Januari','Februari','Maret','April','Mei','Juni'];

var fotoItems = [];
for (var i = 1; i <= 72; i++) {
    fotoItems.push({
        title: judulList[i % judulList.length] + ' ' + i,
        date: 'Rabu, ' + (i % 28 + 1) + ' ' + bulanList[i % bulanList.length] + ' 2026',
        cat: katList[i % katList.length],
        img: 'https://via.placeholder.com/600x400?text=Foto+' + i,
        jumlahFoto: (i % 20) + 5,
        href: '#'
    });
}
...
var currentPage = 1;
var filteredItems = fotoItems.slice(); // copy semua item
var TOTAL_PAGES = Math.ceil(filteredItems.length / ITEMS_PER_PAGE);

function renderCards(page) {
    var grid = document.getElementById('foto-grid');
    var start = (page - 1) * ITEMS_PER_PAGE;
    var items = filteredItems.slice(start, start + ITEMS_PER_PAGE);

    if (items.length === 0) {
        grid.innerHTML = '<div class="col-span-3 text-center py-20 text-outline">Tidak ada data ditemukan.</div>';
        return;
    }

    grid.innerHTML = items.map(function(item) {
        return '<div class="group bg-white rounded-lg shadow-sm border border-outline-variant/30 overflow-hidden hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">' +
            ...
            '<span class="material-symbols-outlined text-[14px]">photo_library</span> ' + item.jumlahFoto +
            '</div></div>' +
            '<div class="p-4">' +
            '<p class="text-secondary-container font-bold text-[11px] mb-1.5 uppercase tracking-wide">' + item.date + '</p>' +
            '<h4 class="font-headline font-bold text-sm text-primary leading-snug line-clamp-3">' +
            '<a href="/foto/halamanG' + item.href + '">' + item.title + '</a>' +
            '</h4></div></div>';
    }).join('');
}
...
function renderPagination(page) {
    ...
    var btnBase = 'w-10 h-10 flex items-center justify-center rounded font-bold transition-colors cursor-pointer border ';
    var btnActive = btnBase + 'bg-primary text-white border-primary shadow-sm';
    var btnNormal = btnBase + 'bg-white border-outline-variant/50 text-on-surface hover:bg-surface-container-low';
    var btnNav   = btnBase + 'bg-surface-bright border-outline-variant/50 text-on-surface hover:bg-surface-container';
    var btnDisabled = btnBase + 'bg-surface-bright border-outline-variant/30 text-outline cursor-not-allowed opacity-50';

    var html = '';

    // Tombol Prev
    if (page <= 1) {
        html += '<span class="' + btnDisabled + '"><span class="material-symbols-outlined text-sm">arrow_back</span></span>';
    } else {
        html += '<button class="' + btnNav + '" onclick="goPage(' + (page-1) + ')"><span class="material-symbols-outlined text-sm">arrow_back</span></button>';
    }
    ...
    pages.forEach(function(p) {
        if (p === '...') {
            html += '<span class="px-2 text-outline flex items-center">...</span>';
        } else {
    ...
    // Tombol Next
    if (page >= TOTAL_PAGES) {
        html += '<span class="' + btnDisabled + '"><span class="material-symbols-outlined text-sm">arrow_forward</span></span>';
    } else {
        html += '<button class="' + btnNav + '" onclick="goPage(' + (page+1) + ')"><span class="material-symbols-outlined text-sm">arrow_forward</span></button>';
    }
...
function goPage(page) {
    ...
    window.scrollTo({ top: document.getElementById('foto-grid').offsetTop - 120, behavior: 'smooth' });
}

function applyFilter() {
    var kat = document.getElementById('filter-kategori').value;
    var tgl = document.getElementById('filter-tanggal').value;
    filteredItems = fotoItems.filter(function(item) {
        var matchKat = kat === '' || item.cat === kat;
        return matchKat;
    });
    TOTAL_PAGES = Math.ceil(filteredItems.length / ITEMS_PER_PAGE) || 1;
    goPage(1);
}
...
function resetFilter() {
    ...
    filteredItems = fotoItems.slice();
    TOTAL_PAGES = Math.ceil(filteredItems.length / ITEMS_PER_PAGE);
    goPage(1);
}
...
function updateClock() {
    ...
    var tz = offset === 7 ? 'WIB' : offset === 8 ? 'WITA' : offset === 9 ? 'WIT' : '';
    var dateString = dayName + ', ' + day + ' ' + monthName + ' ' + year;
    var timeString = hours + ':' + minutes + ':' + seconds + ' ' + tz;
    var dd = document.getElementById('desktop-date');
    var dt = document.getElementById('desktop-time');
    var md = document.getElementById('mobile-date');
    var mt = document.getElementById('mobile-time');
    if (dd) dd.textContent = dateString;
    if (dt) dt.textContent = timeString;
    if (md) md.textContent = dateString;
    if (mt) mt.textContent = timeString;
}
